...
void servicioAsignacionDeRecursos(void) {
    printf("Servcio de asigncion de memoria es la funcion 'malloc', esa funcion pide memoria al sistema operativo\n");
    sleep(2);
...
void servicioDeManejoDeProcesos(void) {
    printf("Servcio de manejo de procesos lafuncion 'fork', crea un proceso nuevo.\n");
    int proceso_id = fork();
...
void servicoManejoDeErores(int status, const char* mensaje) {
    ...
    printf("Servcio de manejo de error la funcion 'perror', produce un mensaje de error.\n");
    sleep(2);
    if (status > 0) {
        perror(mensaje);
    }
...
void servicioManejoDeArchivos(void) {
    ...
    printf("Servcio de manejo de archivos la funcion 'chmod', permite cambiar los permisos de accesos.\n");
    sleep(2);
...
void servicioDeContabilidad(void) {
    printf("Servcio de manejo de contabilidad la funcion 'getrusage', permite ver la recursos usados.\n");
    sleep(2);
    struct rusage uso;
